-- Waste to Work — 01_schema_rls.sql
-- Schema, enums, sequences, indexes, GRANTs, RLS and policies.
-- Run FIRST, on an EMPTY Supabase project (public schema with no Waste to Work tables).
-- Source: supabase/migrations/20260811164133 (verbatim, guard added).

DO $guard$
BEGIN
  IF EXISTS (SELECT 1 FROM information_schema.tables
              WHERE table_schema = 'public' AND table_name IN ('branches','customers','sessions')) THEN
    RAISE EXCEPTION 'Waste to Work tables already exist in this database. Aborting to avoid clobbering data.';
  END IF;
END $guard$;
